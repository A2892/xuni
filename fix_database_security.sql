...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'courses') THEN
    IF EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'courses' AND column_name = 'user_id') THEN
      ...
      EXECUTE 'CREATE POLICY（如果表存在且有user_id列）
DO $$ 
BEGIN
...
CREATE POLICY "universities_insert_own" ON public.universities
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "universities_update_own" ON public.universities
  FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "universities_delete_own" ON public.universities
  FOR DELETE
  USING (auth.uid() = user_id);
...
BEGIN
  ...
  END IF;
END $$d_documents_admin_all" ON public.saved_documents;
CREATE POLICY "saved_documents_admin_all" ON public.saved_documents
  FOR ALL
  USING (public.is_admin());
...
-- 使用方法：SELECT public.create_admin_user('your-user-id');
...
-- 2. 执行后，使用以下命令将当前登录用户设置为管理员：
--    SELECT public.create_admin_user(auth.uid());
-- 3. 或者指定特定用户ID：
--    SELECT public.create_admin_user('用户UUID');
